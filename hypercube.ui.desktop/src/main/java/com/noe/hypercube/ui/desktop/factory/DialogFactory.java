...
public final class DialogFactory {
    ...
    public static Dialog createProgressDialog(ProgressAwareInputStream stream) {
        final Dialog dialog = new Dialog(null, "Copy file", false, DialogStyle.NATIVE);
        final ProgressBar pb = new ProgressBar(0);
        final ProgressIndicator pi = new ProgressIndicator(0);

        stream.setOnProgressListener( ( percentage, tag ) -> {
            pb.setProgress(new Double( percentage)/100);
            pi.setProgress(((double) percentage)/100);
            if(percentage == 100) {
                dialog.hide();
            }
        } );
